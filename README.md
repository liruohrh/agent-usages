...
**价格表用什么货币报价就存什么货币**（`price` 打印的永远是厂商原价）。报告在**开始时转换一次**——把价格表里的单价整个折算成显示货币，之后金额和单价都是同一种货币，不存在"金额转了、单价没转"这种半截状态。

```bash
agent-usages usage                      # 按系统语言：中文环境 ¥、英文环境 $
agent-usages usage --currency USD       # 用内置汇率表折算成美元
agent-usages usage --currency USD --currency-rate 0.15   # 用你给的汇率
agent-usages usage --currency-rate 0.5  # 只给汇率：照常折算，但不显示货币符号
```

报告头部会写清用了什么货币、什么汇率、汇率来自哪里：
...
- 默认币种：系统语言的货币；语言没有意见时优先美元，再不行就保持厂商原币。
- 汇率表以美元为基准，任意两种货币互算；内置的是一份**种子表**（带日期、标明来源），联网更新与缓存见后续版本。
- `--currency-rate` 只接受正的十进制数；`--json` 里给出 `currency`、`currencyRate` 与 `rateInfo`（基准币种、显示币种、汇率、来源、日期）。
