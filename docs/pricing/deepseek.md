...
时段都是固定偏移：中文站写北京时间（UTC+8），英文站直接写 UTC（01:00–04:00、06:00–10:00），
两者对应同一批时刻。价格表里因此只记 `from` 的偏移，不写时区名。
...
| 站点 | 币种 | 来源 |
| --- | --- | --- |
| 中文站 | CNY | `https://api-docs.deepseek.com/zh-cn/quick_start/pricing` 及其历史快照 |
| 英文站 | USD | `https://api-docs.deepseek.com/quick_start/pricing` 及其历史快照 |

美元价是人民币价按 2~3 位有效数字圆整的结果（例如当前 Flash 输出 4 元 → $0.6），并不等于
当时的汇率换算值，因此两套都按原样收录，各自带来源 URL 与期间。

报告只挑一套来算：`--currency` 指定的币种若厂商发布了就用它，否则按系统语言（中文 → 人民币表、
英文 → 美元表），再不然优先美元表。**用发布表时不折算**；只有当你要的币种厂商没发布时（如
`--currency EUR`）才从美元表折算，并写明汇率与来源。折算发生在计价之前，单价与金额一起换算。
