...
| 会话 | **横向对比**：每个会话一行（跨项目时带项目列），点表头按 会话/项目/agent/Q/I-C/缓存金额/T/费用/最后使用 排序；「展开分桶」出现 I/M、I/W、O、R |
...
- 会话明细表的「合并子代理 / 含子代理」对应 CLI 默认与 `--subagents`；表头可点，
  默认按**费用降序**（"哪个会话最花钱"是第一个问题），再点一次翻转方向；
  概览页另有「花费最多的会话」前八名，一眼看出哪个项目的哪个会话最贵、缓存了多少；
