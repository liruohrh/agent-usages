...
- **「饼图分析」按钮（默认收起）**：点开后每个指标一张饼，回答的是另一个问题——
  "这个总数是谁贡献的"。以项目页为例就是所有项目的 `I/M` 各占多少、`I/C`、`O`、`总 token`、
  `缓存金额`、`费用`……每张饼按列表条目切，只列前五项，其余合并为"其他"（29 个会话画一张饼
  读不出来），旁边附图例（名字 / 数值 / 占比）。时间类指标（最近）不做饼，时间戳相加没有意义；
