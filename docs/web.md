...
- **条的算法**（图例上的 ⓘ 有原文）：条长 = 这一行的「当前排序指标」÷ 列表里**最大那一行**
  的同一个指标——最大的那行占满，其它行与它比；**不是**占合计的比例（那是费用后面的小字）。
  分段 = 这一行自己的四个计费桶构成（I/M、I/C、I/W、O，按 token 占比），每个非零桶至少保留
  1.2% 宽度（缓存命中常占 99% 以上，等比分段会把其它颜色压成 0 像素）。每行下方给色块标签
  与确切值，缓存命中率直接跟在 `I/C` 后面；思考是输出的一部分，不单独成列，展开后有一行；
