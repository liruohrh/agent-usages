...
| 会话 | **横向对比**：每个会话一行（跨项目时带项目列），默认就把 Q、I/M、I/C（含命中率）、缓存金额、O、R、T、费用（含占比）、最后使用 全列出来，点任意表头排序；「精简列」只留主列（窄屏用） |
...
- 会话明细表默认**所有指标都在**（列不藏在开关后面，只在整列为 0 时才不出现，
  例如没发生过缓存写入就不显示 I/W）；表头可点，默认按**费用降序**（"哪个会话最花钱"
  是第一个问题），再点一次翻转方向；「合并子代理 / 含子代理」对应 CLI 默认与 `--subagents`；
  概览页另有「花费最多的会话」前八名，同样带各桶与缓存金额；
...
pnpm web:e2e                                        # 真浏览器：18 条，起服务 + 切标签/项目/会话 + 排序 + 口径与布局
