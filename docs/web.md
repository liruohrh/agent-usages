...
- **构图的算法**：条长 = 该行在当前排序指标上相对榜首的比例（榜首满长）；分段 = 这一行
  自己的四个计费桶构成（I/M、I/C、I/W、O，按 token 占比），并且**每个非零桶至少保留
  1.2% 宽度**——缓存命中常占 99% 以上，等比分段会把其它颜色压成 0 像素（上一版看起来
  "只有紫色"）。每行下方给色块标签与确切值，缓存命中率直接跟在 `I/C` 后面；思考不再单独
  成列（它是输出的一部分，展开后有一行）；
- 右侧大字是当前指标值，下面一行是该行的 token 合计（不写 `T` 前缀，也不再追加
  "（合计 …）"——列尾已经把两件事说完了）；
