...
- **每行两条，各回答一个不同的问题**（表头下方有一行原文说明）：
  **上条 = 大小**，长度 = 这一行的「当前排序指标」÷ 列表里**最大那一行**的同一个指标
  （最大的那行占满；**不是**占合计的比例，那个比例写在费用后面的小字里）；
  **下条 = 构成**，永远是整宽，按这一行的四个计费桶（I/M、I/C、I/W、O）token 占比切开，
  整条就是这一行的 100%；每个非零桶至少保留 1.2% 宽度（缓存命中常占 99% 以上，等比分段
  会把其它颜色压成 0 像素）。
  上一版把这两件事画进同一条，于是每段宽度 = 占比 × 行长度，是个没有意义的乘积——这就是
  "看不懂"的原因。每行下方给色块标签与确切值，缓存命中率直接跟在 `I/C` 后面；思考是输出的
  一部分，不单独成列，展开后有一行；
