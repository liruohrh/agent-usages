...
| 标签 | 回答什么 |
| --- | --- |
| 概览 | 花了多少、花在哪：四张大数字卡（钱 / `T` / `I/C 缓存` / `Q`，一张只说一件事）、构成条、时间序列，下面是「项目（前五）」「会话（前五）」两张榜单 |
...
| 会话 | **排行榜**：每个会话一行 —— 排名、标题/项目/agent、一条按计费桶着色的构图条（整条 = 本行自己的构成）、条下每个桶带自己的 tokens 与钱、右侧三列 `Q` / `T` / 钱（无列名）；点一行展开该会话的**全部计费桶（tokens、占 `T`、钱）**与首末时间、自身/子代理；排序走「排序」下拉（Q、I/M、I/C、I/W、O、R、T、费用、最近）配升降序，另存一个「表格视图」给习惯看列的人 |
...
- 「完整指标」展开后是 CLI 的十项数字 `I/M I/W I/C I/T O R O/T T Q` 与合计，
  **每项一列**，数量在上、费用与占比在下 —— 可直接逐列对照终端输出；
- 项目 / agent / 工作区 / 会话**都用同一套排行榜行**：排名、名字、计费桶色块标签、
  一条按计费桶着色的构图条、条下每个桶自己的 token 与钱、右侧三列 `Q` / `T` / 钱（无列名），
  点开是该实体的**全部计费桶（tokens、占 `T`、钱）**。不再需要"这张表看完了换那张表"；
- **行的三列固定不变**：右边依次是 `Q`、`T`、钱——**钱那一列没有列名**，格子本身就是
  `¥33.2314`（后面跟着它占本列表合计的比例）；换排序只改顺序，数字不挪位置，正在排序的
  那一列会高亮；
...
- **数字在条下，每个桶都带自己的钱**：`I/M 224万 ¥2.3398`、`I/C 9.7亿 99.8% ¥19.4932`、
  `I/W …`、`O 281.7万 ¥11.3984`、`R …`（后两者有才出现）；总和类的 `T` 与行尾那笔钱，
  以及 `I/C` 那一项的钱本身（＝缓存命中的钱）**不在条下重复**——它们在右侧三列里；
- **排序只走右上角的下拉**（列出全部指标：`Q`、`I/M`、`I/C`、`I/W`、`O`、`R`、`T`、
  `费用`、`最近`）配降序/升序按钮；数字本身不可点，避免"点哪都能排序"的误触；
- 右侧固定三列 `Q` / `T` / 钱（钱后面是占本列表的比例），方便竖着扫；
- **「图表分析」按钮（默认收起）**：点开后每个指标一张**横向柱状图**，回答另一个问题——
  "这个总数是谁贡献的"。以项目页为例就是所有项目的 `I/M` 各占多少，`I/C`、`O`、`R`、
  `T`、钱 各一张。条长是该条目占这个指标总量的比例（每张图都"占满 = 全部"），
  右侧给数值与占比。卡片里只列前 6 项、其余在下面给合计，**「展示更多（全部 N 项）」**
  打开一个弹窗列出该指标的**每一个条目**（同一个横向柱状图，只是放大了、不再截断）：
  弹窗里每个条目**两行**——上面是名字、条、数值、占比，下面**缩进一行**是这个条目的
  其他计费桶（`tokens` + 它自己的钱，与榜单行条下那行同一个组件），正在画的这个指标
  不重复（它已经是上面那条和那个数字）；按 Esc、点背景或点「关闭」收起。饼图试过：
  29 个条目切成 29 片读不出来，横向柱状图还能顺着名字和数字比大小；
- **钱不写名字**：术语只有「费用」一个词（不再出现"金额"、"缓存金额"），而且**只在需要一个
  "名字"的地方才写**——排序下拉的选项、时间序列的切换按钮、agent 表的分桶模式。表格里的
  钱列一律留空表头（悬停有说明），四张大数字卡里那张也不写标签：`¥12.34` 的 `¥` 就是它的
  名字；**"缓存金额"这类派生指标不单列**——它就是 `I/C` 那一项的钱，已经在条下的 `I/C` 里；
  时间类指标（最近）不做图表，时间戳相加没有意义；
- 树里的行保持一行高（340px 放不下整行），**悬停**给出该行完整的指标行，选中的行
  在树下给两行「自身 / 子代理」。

### 指标缩写（与 CLI 同一套）

指标名只写 CLI 的缩写，**页面上不出现中文名**（`I/C` 在卡片上多带一个词：
`I/C 缓存`，因为它那一格是"输入里有多少来自缓存"这个比例，不是 token 数）：一个数字在终端和浏览器里长得一样，
才认得出是同一个。中文名是**定义**，只出现在三处——悬停的 `title`、这里的文档、
`--help`，所以每张表都能对照回来：

| 缩写 | 是什么 | 计价 |
| --- | --- | --- |
| `Q` | 请求数 | — |
| `I/M` | 未命中缓存的输入（input miss） | 独立计费项 |
| `I/C` | 缓存命中输入（cache read） | 独立计费项，旁边的小字是 `I/C ÷ I/T`；卡片上这一项写 `I/C 缓存` |
| `I/W` | 缓存写入输入（cache write） | 独立计费项，全为 0 时不出现 |
| `I/T` | 输入合计 = `I/M + I/C + I/W` | 是上面三项的相加，不是第二笔账 |
| `O` | 输出里不含思考的那部分 | 与 `R` 分输出这一笔账（`O + R = O/T`） |
| `R` | 思考 token（输出的一部分） | 全为 0 时不出现；不另行计费 |
| `O/T` | 输出合计 = `O + R` | 是上面两项的相加 |
| `T` | 计费桶 token 合计 = `I/T + O/T` | 自身不带价：它的钱就是行尾的费用总额 |
| `最近` | 最后一次计费请求的时间 | 时间类指标不做图表（时间戳相加没有意义） |
| — | 钱（费用） | 没有缩写，也不写列名：`¥12.34` 自带货币符号。只有"取值"的地方才写「费用」 |

钱只有「费用」一个词（不再有"金额"、"缓存金额"），而且表格列头、数字卡上都不写它：
`¥12.34` 里的 `¥` 已经说明这是钱。`I/C 缓存` 是缓存命中率，分母是 `I/T`（与 CLI 印在
`I/C` 后面的那个比例同口径）。`pnpm web:e2e` 里有一条用「页面文字里不许出现中文指标名」
把这条规则钉住的用例——把缩写改回中文名会红。
