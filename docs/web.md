...
- 会话不再用宽表：十几个数字并排，眼睛没法跨行相减。改成**排行榜**——条形的长度就是
  当前排序指标，分段就是计费桶，形状本身完成比较；精确数字点开即见，且默认排序是
  花费（"哪个会话最花钱"是第一个问题），「合并子代理 / 含子代理」对应 CLI 默认与
  `--subagents`；概览页的「花费最多的会话」用的是同一种行，前五名；
...
pnpm web:e2e                                        # 真浏览器：19 条，起服务 + 切标签/项目/会话 + 排序 + 口径与布局
