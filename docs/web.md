...
| 标签 | 回答什么 |
| --- | --- |
| 概览 | 花了多少、花在哪：四张大数字卡（费用 / `Q` / `T` / `I/C 占比`）、构成条、时间序列，下面是「项目（前五）」「会话（前五）」两张榜单 |
...
| 会话 | **排行榜**：每个会话一行 —— 排名、标题/项目/agent、一条按计费桶着色的构图条（整条 = 本行自己的构成）、条下每个桶带自己的 tokens 与费用、右侧三列 `Q` / `T` / 费用；点一行展开该会话的**全部计费桶（tokens、占比、费用）**与首末时间、自身/子代理；排序走「排序」下拉（Q、I/M、I/C、I/W、O、R、T、费用、最近）配升降序，另存一个「表格视图」给习惯看列的人 |
...
- 项目 / agent / 工作区 / 会话**都用同一套排行榜行**：排名、名字、计费桶色块标签、
  一条按计费桶着色的构图条、条下每个桶自己的 token 与费用、右侧三列 `Q` / `T` / 费用，
  点开是该实体的**全部计费桶（tokens、占 `T`、费用）**。不再需要"这张表看完了换那张表"；
- **行的三列固定不变**：右边依次是 `Q`、`T`、`费用`（费用后面的小字是它占本列表
...
- **数字在条下，每个桶都带自己的费用**：`I/M 224万 ¥2.3398`、`I/C 9.7亿 99.8% ¥19.4932`、
  `I/W …`、`O 281.7万 ¥11.3984`、`R …`（后两者有才出现）；总和类的 `T`、`费用`，
  以及 `I/C` 那一项的费用本身（＝缓存命中的钱）**不在条下重复**——它们在右侧三列里；
- **排序只走右上角的下拉**（列出全部指标：`Q`、`I/M`、`I/C`、`I/W`、`O`、`R`、`T`、
  `费用`、`最近`）配降序/升序按钮；数字本身不可点，避免"点哪都能排序"的误触；
- 右侧固定三列 `Q` / `T` / `费用`（费用后面是占本列表的比例），方便竖着扫；
- **「图表分析」按钮（默认收起）**：点开后每个指标一张**横向柱状图**，回答另一个问题——
  "这个总数是谁贡献的"。以项目页为例就是所有项目的 `I/M` 各占多少，`I/C`、`O`、`R`、
  `T`、`费用` 各一张。条长是该条目占这个指标总量的比例（每张图都"占满 = 全部"），
...
金额只有「费用」一个词（不再有"金额"、"缓存金额"）；`I/C 占比` 是缓存命中率，
分母是 `I/T`（与 CLI 印在 `I/C` 后面的那个比例同口径）。`pnpm web:e2e` 里有一条
用「页面文字里不许出现中文指标名」把这条规则钉住的用例——把缩写改回中文名会红。
