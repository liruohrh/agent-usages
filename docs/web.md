...
| 标签 | 回答什么 |
| --- | --- |
| 概览 | 花了多少、花在哪：四张大数字卡（钱 / `Q` / `T` / `I/C 占比`）、构成条、时间序列，下面是「项目（前五）」「会话（前五）」两张榜单 |
...
指标名只写 CLI 的缩写，**页面上不出现中文名**：一个数字在终端和浏览器里长得一样，
...
| `I/C` | 缓存命中输入（cache read） | 独立计费项，旁边的小字是 `I/C ÷ I/T` |
...
`¥12.34` 里的 `¥` 已经说明这是钱。`I/C 占比` 是缓存命中率，分母是 `I/T`（与 CLI 印在
