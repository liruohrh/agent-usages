...
| 层 | 位置 | 谁写 |
| --- | --- | --- |
| 仓库默认 | `config/pricing.json`、`config/rates.json`、`config/holidays.json`（随包发布） | 维护者提交 |
...
## 节假日表（`config/holidays.json`）

有些厂商按**工作日历**而不是星期几定价：DeepSeek 的峰谷规则是「周一至周五 09:00-12:00、14:00-18:00（UTC+08:00），
**排除中国法定节假日**；其余时间——包括周末与节假日全天——按低谷价」。节假日落在工作日时，那一天的单价是高峰价的一半，
差的是真钱，所以日期必须有出处。
...
```jsonc
{
  "version": 1,
  "zone": "Asia/Shanghai",
  "source": "国务院办公厅关于2026年部分节假日安排的通知（2025-11-04，中国政府网）",
  "days": { "2026-10-01": "国庆节", "2026-10-05": "国庆节" }
}
```

| 规则 | 说明 |
| --- | --- |
| 只列**放假**的日子 | 含调休挪出来的休息日。调休上班的周末不列：我们的规则只在周一到周五开高峰窗口，周末（含调休上班的周末）本来就全天低谷——DeepSeek 后来也补充说明「**调休上班的周末、中国法定节假日全天均按空闲时段计费**」，两者一致。哪天有厂商改成「按工作日计价」，再把这些日期单独收进来 |
| 口径是"放假期间" | 2026 年中秋+国庆的「十天半价窗口」（9/25–9/27、10/1–10/7）与本表逐日一致；不是只算《放假办法》里的 13 天法定节假日 |
| 为什么不做算法 | 春节、端午、中秋按农历，且每年的放假调休安排由国务院公告，**算不出来**；所以它是数据，和价格表一样人工维护 |
| 生效方式 | 价格区间里写 `"holidayCalendar": "cn"` 才生效（不写＝只管星期几）；写错编号会报错，不会静默忽略 |
| 覆盖范围 | `days` 的最早/最晚日期。某个生效中的区间超出了这个范围，配置会带一条**警告**：之后按星期几判定，节假日会被按高峰价计——所以要提醒你去更新它，而不是悄悄多收钱 |
| 更新 | 与价格表同仓库、同一条 3 周一次的检查；拉到的文件先过校验再落盘，坏了就退回随包那份并给一条提示 |
| 校验 | `check-config` 会检查它：日期必须是真实的 `YYYY-MM-DD`、名称非空、至少要有一天 |

给一年补日期通常在 11 月——国务院一般在那时公布次年的放假安排。`agent-usages price` 会把这条规则写在区间下面：
`峰谷: 周一至周五 09:00-12:00、14:00-18:00（UTC+08:00）；中国法定节假日全天低谷（已收录 2026-01-01 ~ 2026-10-07）`。
