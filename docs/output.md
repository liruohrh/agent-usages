...
- 会话表与项目行的列是 **token 五桶**（`I/M`、`I/C`、`I/W`、`O`、`R`，与文本同一套标签）+ `T` + `Q` + 钱，会话表末尾另有首次/最近日期。**钱那一列没有表头**：每一格都是 `¥12.34`，货币符号自己说明它是钱（文本报告的指标行也是这么结束的——`… T 1.21B · Q 4,043 · ¥6.0946`），标题行里的 `金额` 只会重复它。
